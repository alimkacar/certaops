...
from ..privacy import RetentionSweeper
...
_agents: dict[str, SAPMultiAgent] = {}
...
def _agent_for(actor: ActorContext, session_id: str) -> SAPMultiAgent:
    """Oturuma bagli agent.
...
def health() -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        ...
        "registered_agents": len(agent_catalogue()),
...
@app.get("/agents", summary="SAP domain agent katalogu")
def agents(actor: ActorContext = Depends(current_actor)) -> dict[str, Any]:
    return {
        "architecture": "orchestrator + isolated domain agents",
        "actor": actor.to_dict(include_scopes=True),
...
def chat(
    ...
    try:
        record, _ = _session_store.get_or_create(request.session_id, actor=actor)
